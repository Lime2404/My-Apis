...
// 15:24 https://youtu.be/eSYSQikJCTY?list=PLS-CH047rQ3U8iIUIb9gm3cyf9kbOSasP
    ...
    public void openAllHrefs3() {
        ...
        // промежуточный метод collect преобразует отфильтровынные значение например в список ( фиксирует результат)
        List<Integer> linksLenght = hrefs.stream().map(x->x.getAttribute("href").length()).collect(Collectors.toList());
    int i = 0;
        // выше мы получаем новый список с числами, означающие длину ссылки
        // этот пример был про то что можно использовать streaм() для работы с коллекциями в селениде
    }
}
